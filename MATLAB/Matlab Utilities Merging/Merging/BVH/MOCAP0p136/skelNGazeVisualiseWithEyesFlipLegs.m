function handle = skelNGazeVisualiseWithEyesFlipLegs(channels, skel,gazeData,headRot, padding,handle)
handle=[];
...
vals(29,:)=vals(25,:)+gazeData.Right.endPoint'*headRot; %left gaze endpoint
vals(30,:)=vals(25,:)+gazeData.Left.endPoint'*headRot; %right gaze enedpoint
...
handle(5) = plot3(vals([32], 1), vals([32], 3), vals([32], 2), 'm.');hold on


%axis ij % make sure the left is on the left.
set(gca,'xdir','reverse') % does same as above, but more vobose
